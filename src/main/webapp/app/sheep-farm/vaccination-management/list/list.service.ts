...
@Injectable()
export class ListSheepVaccinationService {
  loading = true;
  list: any;
  ...
  loadFromServer(): Observable<SheepVaccinationModel[]> {
    return this.http
      .post<Pageable<SheepVaccinationModel>>(
        this.applicationConfigService.getEndpointFor('api/sheep-vaccination/list'),
        this.createBody(),
        {
  ...
  delete(id: number): Observable<SheepVaccinationModel> {
    return this.http.post<SheepVaccinationModel>(this.applicationConfigService.getEndpointFor(`api/sheep-vaccination/delete/${id}`), null);
  }
  ...
  private createBody(): SheepVaccinationCriteria {
    const body: SheepVaccinationCriteria = {
      farmId: {
