...
export class UpgradeSubscriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.pipe(first()).subscribe(paramMap => {
      ...
      if (farmId != null && subscriptionId != null) {
        this.farmId = +farmId;
        this.subscriptionId = +subscriptionId;

        this.service.listSuitablePackagesForUpgrade(this.farmId, this.subscriptionId).subscribe(
          result => {
  ...
  upgrade(): void {
    if (this.form.valid && this.farmId && this.subscriptionId) {
      this.submitting = true;

      this.service.upgrade(this.farmId, this.subscriptionId, this.form.get('packageId')?.value).subscribe(
        () => {
