package com.familytree.service.client;
...
public class ClamAVClient {
    ...
    private static Logger log = LoggerFactory.getLogger(ClamAVClient.class);
    ...
    public byte[] scan(InputStream is) throws IOException, NoSuchAlgorithmException {
    ...
    private byte[] assertSizeLimit(byte[] reply) {
        ...
        if (r.startsWith("INSTREAM size limit exceeded.")) throw new RuntimeException(
            "Clamd size limit exceeded. Full reply from server: " + r
        );
        return reply;
    }
